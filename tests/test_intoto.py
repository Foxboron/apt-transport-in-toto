...
FINAL_PRODUCT_PATH_GOOD = os.path.join(TEST_DATA_PATH, "good",
    "final-product_0.0.0.0-0_all.deb")
FINAL_PRODUCT_PATH_BAD = os.path.join(TEST_DATA_PATH, "bad",
    "final-product_0.0.0.0-0_all.deb")
...
def mock_http():
  ...
  try:
    ...
    intoto.write_one(_MSG_URI_DONE.format(
        filename=dict(acquire_msg["fields"]).get("Filename", "")), sys.stdout)
...
def mock_apt(intoto_proc, config_args=None, acquire_args=None):
  ...
  overriden using arguments config_args and acquire_args.
...
class InTotoTransportTestCase(unittest.TestCase):
  ...
  def tearDown(self):
    """Tell intoto transport to shutdown and wait until it did. We also have
    to close the opened subprocess pipes. """
  ...
  def test_missing_links_nofail(self):
    """Verification passes despite missing links because of nofail option. """
    result = mock_apt(self.intoto_proc,
        config_args={"rebuilder1": "http://127.0.0.1:8083", "no_fail": "true"})
    self.assertEqual(result["code"], intoto.URI_DONE)
